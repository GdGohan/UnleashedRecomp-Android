...
#if defined(SDL_VULKAN_ENABLED) || defined(__ANDROID__)
    // SDL_VULKAN_ENABLED is force-disabled for Android by plume's own
    // cmake_dependent_option (gated on IS_LINUX), so check __ANDROID__ directly here.
