...
TARGET_URL = "http://www.sonicteam.com/movie/swa/*"

# Pasta de destino
PASTA_DESTINO = "download/movies"
...
for item in dados:
    ...
    url_sem_query = url.lower().split("?")[0]

    if not url_sem_query.endswith(".flv"):
        continue

    # Adiciona a URL original
    if url not in urls:
        urls.append(url)

    # Se for _h.flv, adiciona também a versão _f.flv
    if url_sem_query.endswith("_h.flv"):

        url_f = url[:-len(".flv") - 2] + "_f.flv"

        if url_f not in urls:
            urls.append(url_f)


print(f"\nArquivos FLV encontrados: {len(urls)}")
